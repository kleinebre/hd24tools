...
void hd24transferengine::closebuffers(unsigned char** audiobuf,uint32_t channels)
{
#if (HD24TRANSFERDEBUG==1) 
...
bool hd24transferengine::openinputfiles(SNDFILE** filehandle,SF_INFO* sfinfoin,uint32_t channels)
{
	int cannotopen=0;
	if (job==NULL)
	{
...
bool hd24transferengine::openoutputfiles(hd24sndfile** filehandle,uint32_t channels,uint32_t partnum,int prefix)
{
	trackspergroup=0;
...
void hd24transferengine::closeinputfiles(SNDFILE** filehandle,uint32_t channels)
{
...
void hd24transferengine::closeoutputfiles(hd24sndfile** filehandle,uint32_t channels)
{
...
bool hd24transferengine::dontopenoutputfiles(hd24sndfile** filehandle,uint32_t channels,uint32_t partnum,int prefix)
{
	// HACK 
	trackspergroup=0;
...
void hd24transferengine::flushbuffer(hd24sndfile** filehandle,unsigned char** buffer,uint32_t flushbytes,uint32_t channels)
{
	#if (HD24TRANSFERDEBUG==1) 
...
int64_t hd24transferengine::transfer_to_pc()
{
	...
	*/
	...
	for (uint32_t tracknum=0;tracknum<MAXPHYSICALCHANNELS;tracknum++)
	{
...
*/
		...
		*/
		...
		cout << "Writing file audio to HD24 buffer, "
			...
			<< endl;
#endif
		for (uint32_t sam=0;sam<maxcount;sam+=trackchans)
		{
			int whichsam=sam;
			...
			if (action==2)
			{
				...
				if (sam<samsread)
				{
					...
					else
					{
						...
						for (uint32_t whichchan=0;whichchan<trackchans;whichchan++) {
		...
		} // end for (uint32_t sam=0;sam<maxcount;sam+=trackchans)
...
///////// START: VARIOUS INIT /////////////////
	...
	for (uint32_t currchan=0;currchan<MAXPHYSICALCHANNELS;currchan++) 
	{
...
/////// RECORD MODE ENABLED //////////
	...
	for (uint32_t fh=0;fh<logical_channels;fh++) {
...
//		_generate_smpte(wamplesperlogicalchannel,wamsincurrblock,wamplenum,&audiodata[0]);
		...
		*/
			...
			for (uint32_t fbyte=0; fbyte<30; fbyte++)
			{
	...
	for (uint32_t bufnum=0;bufnum<MAXPHYSICALCHANNELS;bufnum++)
	{
